...
def compute_losses(batch, orienter_model, flipper_model, flip_matrices, use_flipper_voting_scheme=True, top4=False):
    ...
    chamfer_dist /= xyz.shape[1] 

    return chamfer_dist, xyzs_oriented_flipped, target_rotation_matrices, data_indices
...
def main():
    ...
    results_dir = "benchmark_results/our_pipeline_chamfer_distances_voting_scheme_top1"
    os.makedirs(results_dir, exist_ok=True)
    point_cloud_dir = "benchmark_results/our_pipeline_chamfer_distances_voting_scheme_top1/point_clouds"
    os.makedirs(point_cloud_dir, exist_ok=True)

    for i, batch in enumerate(tqdm(val_dataloader)):
        with torch.no_grad():
            chamfer_dist, xyzs_oriented_flipped, target_rotation_matrices, data_indices = compute_losses(batch, orienter_model, flipper_model, flip_matrices, use_flipper_voting_scheme, top4)
            ...
            point_cloud_oriented = trimesh.Trimesh(vertices=xyzs_oriented_flipped.squeeze(), faces=[])
        ...
        print(f"Mean chamfer dist so far: {np.mean(chamfer_dists)}")
    ...
    # Save losses
    np.save(os.path.join(results_dir, "chamfer_dists.npy"), chamfer_dists.cpu().numpy())
